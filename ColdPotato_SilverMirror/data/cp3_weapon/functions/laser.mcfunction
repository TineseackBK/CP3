...
# 射击：F 键
execute at @s if entity @e[tag=module,tag=weapon,sort=nearest,limit=1,scores={cd_laser=60}] run tag @s[nbt={Inventory:[{Slot:-106b,id:"minecraft:bone"}]}] add shoot_laser
	# cd
	execute at @s[tag=shoot_laser] run scoreboard players set @e[tag=module,tag=weapon,sort=nearest,limit=1] cd_laser 0
